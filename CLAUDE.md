...
- 最小OS: macOS 26.0 / 非サンドボックス（unixソケット・NSWorkspaceアクティベーション・herdr CLI起動のため）
- herdr Socket API: `~/.config/herdr/herdr.sock`、NDJSON JSON-RPC

## リポジトリ構成（SwiftPM ローカルパッケージ）

```
HerdWatch/                       ← リポジトリルート
├── project.yml                  ← XcodeGen定義: macOSアプリ（正本）
├── HerdWatch.xcodeproj          ← 生成物（gitignore済み・手編集禁止）
├── HerdWatch/                   ← macOSアプリ本体
├── HerdWatchTests/              ← macOSアプリのテスト
├── Packages/HerdWatchShared/    ← SwiftPM ローカルパッケージ（Domain・UI・Socket・Companion・Support）
│   ├── Package.swift
│   ├── Sources/HerdWatchShared/
│   └── Tests/HerdWatchSharedTests/ ← 共有コードの単体テスト（swift test で実行可能）
└── docs/adr/                    ← 設計判断の正本
```

- 共有コードは `Packages/HerdWatchShared`（SwiftPM ローカルパッケージ）で管理。パッケージ内の型は `public` で公開。
- Mac専用ファイル（Socket層・Companionホスト・Settings・AppKit UI・HerdWatchApp）は `HerdWatch/` 配下。
- iOS Companion は制作中止。コードは `shogoisaji/HerdWatchIOS` リポジトリ（アーカイブ）へ移動済み。
...
- `project.yml` が正本。`.xcodeproj` は生成物（gitignore済み）。`project.pbxproj` を手で編集しない。
- ローカルはad-hoc署名（`CODE_SIGN_IDENTITY = "-"`）。

## Companionホスト（Mac側・MultipeerConnectivity）

MacアプリはMultipeerConnectivityでCompanionホストとして動作可能（真実源はMacのPastureStoreのまま・ADR-0001準拠）。iOS Companion側は制作中止だが、Mac側のホスト実装・プロトコル型は `HerdWatchShared` に残している。

- 通信プロトコル型は `Packages/HerdWatchShared/Sources/HerdWatchShared/Companion/CompanionProtocol.swift`。NDJSON1行=1JSONメッセージ。`CompanionSnapshot`(Mac→iOS)と`CompanionFocusCommand`(iOS→Mac)を`CompanionMessage`エンベロープで包む。
...
- コード共有: Domain層・スプライト描画(`CharacterSprite`/`SpriteRenderer`/`OverlayBadge`/`AgentAnimator`/`HitTest`)・`AnyRNG`・`HerdrModels`・`WorkingElapsedFormatter`・`CompanionProtocol`は `HerdWatchShared` パッケージ。AppKit依存(`NSWorkspace`/`NSView`/`NSMenu`)・Socket層・Companionホスト・SettingsはMac側のみ。
...
- CI: `.github/workflows/ci.yml`（PR/push で shared tests / macOS build+test）
- リリース: `.github/workflows/release.yml`（`release: published` で署名・notarize・DMG・appcast・Cask更新dispatch）
- Mac App Store は非サンドボックスのため対象外。
- Sparkle: SwiftPM `Sparkle` 2.9.3。`SUPublicEDKey` / `SUFeedURL` は `project.yml` の `info:` で Info.plist 生成。秘密鍵は CI secret `SPARKLE_PRIVATE_KEY_PEM`（絶対にコミットしない）。
